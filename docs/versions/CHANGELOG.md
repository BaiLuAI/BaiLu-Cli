...
#### 問題描述
修復了 CLI 工具中用戶輸入訊息重複顯示的問題。之前當用戶輸入訊息後，介面會顯示兩次相同的訊息。

#### 根本原因
問題出在 `src/agent/chat.ts` 的 `handleSingleLine` 方法中。Windows 平台特定的代碼會導致用戶輸入被顯示兩次，因為 readline 會自動處理輸入的顯示，而代碼又手動重複顯示一次。
